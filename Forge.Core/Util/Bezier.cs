...
namespace Forge.Core.Util{
    ...
    internal class BezierDependentGenerator{
        ...
        readonly int _resolution;
            //represents the number of halfing operations required to get a result with an accuracy of less than one pixel in the worst case
    ...
    internal class BruteBezierGenerator{
        ...
        public Vector2 GetPointFromPointDist(Vector2 originPoint, float distance){
            ...
            if (xMatches.Count > 1){
                throw new Exception
                    ("More than one independent variable detected for the given origin point X value. The curves this class was initalized with do not pass the vertical line test.");
            }
